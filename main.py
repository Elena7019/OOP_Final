...
def strategy(buy_amt, SL = 0.985, Target = 1.02, open_position = False):
    ...
    if ((df.Close.pct_change() +1).cumprod()).iloc[-1] > 1:
        ...
        print(dty)
        order = client.create.order(symbol = asset, side = 'Buy', type = 'MARKET', quantity = dty)
        print(order)
        ...
        while open_position:
            ...
            if df.Close[-1] <= buyprice * SL or buyprice * Target:
                order = client.create_order(symbol = asset, side = 'Sell', type = 'MARKET', quantity = dty)
                print(order)
                break
    else:
                print('No find')
